...
if (!empty($_GET['shop']) && !empty($_GET['code'])) {
    ...
    //---- SEND EMAIL ----
    // require 'plugin/email/install_email.php';ea

    //---- RES WEBHOOK ---- 
    // registerWebhooks($shop, $shopify, $rootLink);
    $webhook =  $shopify('POST', $apiVersion . "/webhooks.json", array('webhook' => array('topic' => 'app/uninstalled', 'address' => $rootLink . "/uninstall.php?shop=$shop", 'format' => 'json')));
    //---- ADD SCRIPTTAG ---- 
    postScriptTag($shop, $appId, $apiVersion, $rootLink, $version);

    if ($chargeType == "free" || $confirmation_url == NULL) {
        db_update("tbl_usersettings", ['confirmation_url' => $confirmation_url], "store_name = '$shop' and app_id = $appId");
        header('Location: https://' . $shop . '/admin/apps/holiday-effects-by-omega');
    } else {
...
function registerWebhooks($shop, $shopify, $rootLink)
{
    ...
    //     pr($webhook);
    // }
    // $webhook =  $shopify('POST', $apiVersion . "/webhooks.json", array('webhook' => array('topic' => 'app/uninstalled', 'address' => $rootLink . "/webhooks/uninstall.php?shop=$shop", 'format' => 'json')));
