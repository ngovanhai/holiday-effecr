...
if (isset($_GET["action"])) {
    ...
    if ($action == "updateAllShopScripTag") {
        $res = db_fetch_array("SELECT * FROM tbl_usersettings WHERE app_id = '34'");
        foreach ($res as $store) {
            $shop = $store['store_name'];
            $shopify = shopifyInit($db, $shop, $appId);
            ...
            if (isset($res[0]['id'])) {
                ...
                $script_arrayScript = array(
                    'script_tag' => array(
                        'event' => 'onload',
                        'src' => $rootLink . '/front-end/effect.js?v=' . time()
                    )
                );
                $updateScript =   $shopify('PUT',  $apiVersion . '/script_tags/' . $id . '.json', $script_arrayScript);
            }
        }
    }
    if ($action == "checkSettings") {
        ...
        $setting = db_fetch_row("SELECT * FROM holiday_events_effects_settings WHERE shop = '$shop'");
    ...
    if ($action == "updateScriptTag") {
        ...
        if (isset($res[0]['id'])) {
            ...
            $script_arrayScript = array(
                'script_tag' => array(
                    'id' => $id,
                    'src' => 'https://apps.omegatheme.com/holiday-effects/front-end/effect.js?v=' . time()
                )
            );
            $updateScript =   $shopify('PUT',  $apiVersion . '/script_tags/' . $id . '.json', $script_arrayScript);
            pr($updateScript);
