...
var rootlinkHoliday = "http://apps.omegatheme.com/holiday-effects";
...
function ot_omgholiday_insertSetting() {
  return new Promise((resolve) => {
    $.ajax({
      url: `${rootlinkHoliday}/admin/server/product.php`,
      type: "GET",
...
function ot_omgholiday_checkCss() {
  return new Promise((resolve) => {
    $.ajax({
      url: `${rootlinkHoliday}/admin/server/product.php`,
      type: "GET",
...
function ot_omgholiady_checkSetting() {
  return new Promise((resolve) => {
    $.ajax({
      url: `${rootlinkHoliday}/admin/server/product.php`,
      type: "GET",
...
function ot_omgholiday_loadFile(result) {
  ...
  <link href='${rootlinkHoliday}/front-end/css/style.css?v=${version}'  rel="stylesheet" />
  `);
...
function ot_omgholiday_renderEvent(key, event) {
  ...
  if (event.choose_icons !== "") {
    ...
    if (event.choose_icons !== "[]") {
      for (var j = 0; j < number; j++) {
        const icons = JSON.parse(event.choose_icons)
        for (icon in icons) {
          if (icons[icon] !== "") {
            divEffects.append(
  ...
  if (event.choose_images !== "") {
    ...
    for (var j = 0; j < event.number_of_icons; j++) {
      if (event.choose_images !== "[]") {
        const images = JSON.parse(event.choose_images)
        for (image in images) {
          if (images[image] !== "") {
            divEffects.append(
  ...
  if (event.custom_images !== "") {
    ...
    for (var j = 0; j <= number; j++) {
      if (event.custom_images !== "[]") {
        const custom_Images = JSON.parse(event.custom_images)
        for (image in custom_Images) {
          if (custom_Images[image] !== "") {
            divEffects.append(
